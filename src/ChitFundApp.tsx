...
import { Calculator, TrendingUp, DollarSign, Users, Globe, AlertCircle } from 'lucide-react';
...
const ChitFundApp = () => {
  ...
  const areAllInputsFilled = (): boolean => {
    ...
    // Check if values are empty (empty string)
  ...
  const calculateChitDetails = () => {
    // Use default values for calculation if inputs are empty
    const totalMembers = getInputValue(calcInputs.totalMembers, defaultValues.totalMembers);
    const monthlyContribution = getInputValue(calcInputs.monthlyContribution, defaultValues.monthlyContribution);
    const firstWithdrawal = getInputValue(calcInputs.firstWithdrawal, defaultValues.firstWithdrawal);
    const monthlyIncrement = getInputValue(calcInputs.monthlyIncrement, defaultValues.monthlyIncrement);
    const commissionRate = getInputValue(calcInputs.commissionRate, defaultValues.commissionRate);
    const oneTimeCommission = getInputValue(calcInputs.oneTimeCommission, defaultValues.oneTimeCommission);
    const loanInterestRate = getInputValue(calcInputs.loanInterestRate, defaultValues.loanInterestRate);
    const commissionType = calcInputs.commissionType;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        {/* Results Section - Only show when all inputs are filled */}
        {showResults && (
          <>
            {/* Key Metrics */}
